...
export async function validateOAuthState(
	...
	if (dotIndex === -1) throw new OAuthError("invalid_request", "Invalid state format", 400);
	...
	if (!isValid) throw new OAuthError("invalid_request", "Invalid state signature", 400);
...
export function renderApprovalDialog(request: Request, options: ApprovalDialogOptions): Response {
	...
	const redirectUris = client?.redirectUris?.map((uri) => { const v = sanitizeUrl(uri); return v ? sanitizeText(v) : ""; }).filter(Boolean) ?? [];
...
<head>
  ...
  <style>
    ...
    .detail-value { font-family: monospace; word-break: break-all; font-size: 0.9em; }
    .detail-value a { color: inherit; }
  ...
  <div class="container">
    ...
    <div class="card">
      ...
      <div class="client-info">
        ...
        ${redirectUris.length > 0 ? `<div class="client-detail"><div class="detail-label">Redirect URIs:</div><div class="detail-value">${redirectUris.map((u) => `<div>${u}</div>`).join("")}</div></div>` : ""}
      </div>
      <p>Approving will redirect you to complete authentication with Cloudflare Access.</p>
...
export async function fetchUpstreamAuthToken(params: {
	...
	return [body.access_token as string, body.id_token as string, null];
}
...
async function getApprovedClientsFromCookie(request: Request, cookieSecret: string): Promise<string[] | null> {
	...
	const targetCookie = cookieHeader.split(";").map((c) => c.trim()).find((c) => c.startsWith(`${approvedClientsCookieName}=`));
